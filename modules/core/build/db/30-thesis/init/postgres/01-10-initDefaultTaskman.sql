...
update WF_PROC set name = 'Управление задачами', card_types = ',tm$Task,', available_for_mobile_client = true where jbpm_process_key = 'TaskManagement' and updated_by is null;
...
update WF_PROC_ROLE set name = 'Инициатор', loc_name = '{"captionWithLanguageList":[{"language":"ru","caption":"Инициатор"},{"language":"en","caption":"Initiator"}]}', role_id = (select id from SEC_ROLE where name = 'task_initiator'),assign_to_creator='TRUE' where code = '10-Initiator' and proc_id = (select id from WF_PROC where jbpm_process_key = 'TaskManagement') and updated_by is null;
update WF_PROC_ROLE set name = 'Исполнитель', loc_name = '{"captionWithLanguageList":[{"language":"ru","caption":"Исполнитель"},{"language":"en","caption":"Executor"}]}', role_id = (select id from SEC_ROLE where name = 'task_executor'), order_filling_type = 'P', is_multi_user = 'true' where code = '20-Executor' and proc_id = (select id from WF_PROC where jbpm_process_key = 'TaskManagement') and updated_by is null;
...
update WF_PROC_ROLE set name = 'Наблюдатель', loc_name = '{"captionWithLanguageList":[{"language":"ru","caption":"Наблюдатель"},{"language":"en","caption":"Observer"}]}', role_id = (select id from SEC_ROLE where name = 'task_observer'), order_filling_type = 'P', is_multi_user = 'true' where code = '90-Observer' and proc_id = (select id from WF_PROC where jbpm_process_key = 'TaskManagement') and updated_by is null;
...
update wf_proc_role set sort_order = 3, notify_without_assignment = true where code = '30-Controller' and proc_id = (select id from wf_proc where code = 'TaskManagement') and delete_ts is null;
update wf_proc_role set sort_order = 4, notify_without_assignment = true where code = '90-Observer' and proc_id = (select id from wf_proc where code = 'TaskManagement') and delete_ts is null;
...
^
select assign_card_creator_default('10-Initiator', 'TaskManagement')^
...
if(cnt = 0) then
insert into SYS_CONFIG (id, create_ts, created_by, version, name, value_) values ('f6a9be40-16ac-11e2-8783-7b7ddae9940d', '2010-10-28 00:00:00.00', 'admin', 1, 'workflow.sms.smsProviderClassName', 'com.haulmont.thesis.core.app.SmsTrafficProvider');
end if;
