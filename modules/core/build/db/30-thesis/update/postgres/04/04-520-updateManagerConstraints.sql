...
JOIN_CLAUSE = 'left join {E}.roles r'
where GROUP_ID = (select g.ID from SEC_GROUP g where g.NAME like '%Руководитель подразделения%' LIMIT 1) and ENTITY_NAME = 'tm$Task';
...
:session$userId OR r.user.id in :session$departmentMembersIds))',
JOIN_CLAUSE = 'left join {E}.roles r'
where GROUP_ID = (select g.ID from SEC_GROUP g where g.NAME like '%Руководитель подразделения%' LIMIT 1) and ENTITY_NAME = 'tm$TaskGroup';
...
:session$userId OR r.user.id in :session$departmentMembersIds)',
JOIN_CLAUSE = 'left join {E}.roles r'
where GROUP_ID = (select g.ID from SEC_GROUP g where g.NAME like '%Руководитель департамента%' LIMIT 1) and ENTITY_NAME = 'tm$Task';

update SEC_CONSTRAINT set WHERE_CLAUSE = '({E}.subCreator.id=:session$userId OR ({E}.global = true AND {E}.template=true)
OR tgt.task.id in (select t.id from tm$Task t left join t.roles r  where t.substitutedCreator.id =
:session$userId OR r.user.id in :session$departmentMembersIds))',
JOIN_CLAUSE = 'left join {E}.roles r'
where GROUP_ID = (select g.ID from SEC_GROUP g where g.NAME like '%Руководитель департамента%' LIMIT 1) and ENTITY_NAME = 'tm$TaskGroup';
