...
(newid(), now(), 'system', 1, 10, 'ts$ArchivedContract.browse', 1, '7091f5ef-a77a-450a-834a-39406885676e'); -- secretary
...
('bc94a0d4-10ca-11e4-8fb1-07511252f87c', now(), 'system', 1, 'df$Doc', 'join {E}.aclList acl', '({E}.docOfficeData.officeFile.state >= 30 or (acl.user.id = :session$userId or acl.global = true))', 'cff945e4-e363-0dc0-d70d-4b5bdb2a2269');

insert into sec_constraint(id, create_ts, created_by, version, entity_name, join_clause, where_clause, group_id) values
('bfb52f2c-10ca-11e4-bf3e-934403a7308f', now(), 'system', 1, 'df$SimpleDoc', 'join {E}.aclList acl', '({E}.docOfficeData.officeFile.state >= 30 or (acl.user.id = :session$userId or acl.global = true))', 'cff945e4-e363-0dc0-d70d-4b5bdb2a2269');

insert into sec_constraint(id, create_ts, created_by, version, entity_name, join_clause, where_clause, group_id) values
('c2292574-10ca-11e4-857e-bb6e9cc79bd5', now(), 'system', 1, 'df$Contract', 'join {E}.aclList acl', '({E}.docOfficeData.officeFile.state >= 30 or (acl.user.id = :session$userId or acl.global = true))', 'cff945e4-e363-0dc0-d70d-4b5bdb2a2269');
