...
(newid(), current_timestamp, USER, 1, 'df$MeetingDoc', ', ts$CardAcl acl', '{E}.id = acl.card.id and (acl.user.id = :session$userId or acl.department.id in :session$departmentIds or acl.global = true)','8d9ba07c-9ffa-11e1-b99d-8fc5b41c7fbb');
INSERT INTO sec_constraint (id, create_ts, created_by, version, entity_name, join_clause, where_clause, group_id) VALUES
(newid(), current_timestamp, USER, 1, 'df$MeetingDoc', ', ts$CardAcl acl', '{E}.id = acl.card.id and (acl.user.id = :session$userId or acl.department.id in :session$departmentIds or acl.global = true)','9fa89a54-9ffa-11e1-b13e-9f4a54bff17e');
INSERT INTO sec_constraint(id, create_ts, created_by, version, entity_name, join_clause, where_clause, group_id) VALUES
('ce84d65b-6f47-c896-d393-bf3dab98660d', current_timestamp, 'system', 1, 'df$MeetingDoc', 'join {E}.aclList acl', '({E}.docOfficeData.officeFile.state >= 30 and {E}.template = false or (acl.user.id = :session$userId or acl.global = true))', 'cff945e4-e363-0dc0-d70d-4b5bdb2a2269');
INSERT INTO sec_constraint (id,create_ts,created_by,version,update_ts,updated_by,delete_ts,deleted_by,entity_name,join_clause,where_clause,group_id) VALUES
