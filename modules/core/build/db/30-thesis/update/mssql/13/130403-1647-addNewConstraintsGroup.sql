...
INSERT INTO SEC_CONSTRAINT (id,create_ts,created_by,version,update_ts,updated_by,delete_ts,deleted_by,entity_name,join_clause,where_clause,group_id) VALUES ('c3096def-c3eb-4580-b9e6-da6a71407f50', current_timestamp,'admin', 1,current_timestamp, 'admin',null,null, 'tm$TaskPattern',', ts$CardAcl acl','{E}.id = acl.card.id and (acl.user.id = :session$userId or acl.global = true)', 'b3dc60f5-65b2-47ab-8802-9e8929bf8b29');

INSERT INTO SEC_CONSTRAINT (id,create_ts,created_by,version,update_ts,updated_by,delete_ts,deleted_by,entity_name,join_clause,where_clause,group_id) VALUES ('8c876c9a-d973-43ed-8837-b01a8219146d',current_timestamp,'admin',1,current_timestamp,'admin',null,null,'tm$TaskGroup','left join {E}.taskGroupTasks tgt left join {E}.participants p',
...
INSERT INTO SEC_CONSTRAINT (id,create_ts,created_by,version,update_ts,updated_by,delete_ts,deleted_by,entity_name,join_clause,where_clause,group_id) VALUES ('70170bc4-1b83-4829-b51c-17c19cf7c6db', current_timestamp,'admin', 1,current_timestamp, 'admin',null,null, 'tm$TaskPattern',', ts$CardAcl acl','{E}.id = acl.card.id and (acl.user.id = :session$userId or acl.global = true)', '9e44a053-a31f-4edd-b19b-39e942161dd2');

INSERT INTO SEC_CONSTRAINT (id,create_ts,created_by,version,update_ts,updated_by,delete_ts,deleted_by,entity_name,join_clause,where_clause,group_id) VALUES ('b9647fb6-33a6-4240-a07d-c63eb19e5e1d',current_timestamp,'admin',1,current_timestamp,'admin',null,null,'tm$TaskGroup','left join {E}.taskGroupTasks tgt left join {E}.participants p',
'({E}.subCreator.id = :session$userId
or p.id = :session$userId
or {E}.chairman.id = :session$userId
or {E}.secretary.id = :session$userId
