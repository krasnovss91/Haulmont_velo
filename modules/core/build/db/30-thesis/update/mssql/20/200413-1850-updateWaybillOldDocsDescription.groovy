...
postUpdate.add({
    ...
    def updateRuSql = "update WF_CARD " +
            "set description = 'Унифицированная накладная (ТОРГ-12)' + SUBSTRING(description, LEN('Накладная') + 1, LEN(description)) " +
    ...
    def updateEnSql = "update WF_CARD " +
            "set description = 'Unified Waybill (TORG-12)' + SUBSTRING(description, LEN('Waybill') + 1, LEN(description)) " +
            "where id in (select CARD_ID from DF_DOC where DOC_KIND_ID = 'a36454de-7af4-11ea-b49f-8b8e95b826ef') " +
