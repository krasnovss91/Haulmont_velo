...
postUpdate.add({
    ...
    def updateSql = "update d " +
            "set DOC_KIND_ID = 'a36454de-7af4-11ea-b49f-8b8e95b826ef' " +
            "from DF_DOC d " +
            "where d.DOC_KIND_ID = '512e83a3-d8c9-45f6-b0de-d1949a55bced' and d.is_template = 0 " +
            "and exists(select * from WF_CARD cc where cc.ID = d.CARD_ID and cc.HAS_ATTACHMENTS = 1) " +
    ...
    try {
        ...
        def query = em.createNativeQuery("select ID from SYS_CATEGORY where ID = '512e83a3-d8c9-45f6-b0de-d1949a55bced'")
        List list = query.getResultList()
        def oldDocKindExists = !list.isEmpty()
        if (oldDocKindExists) {
            query = em.createNativeQuery("select ID from SYS_CATEGORY where ID = 'a36454de-7af4-11ea-b49f-8b8e95b826ef'")
            ...
            if (!newDocKindExists) {
                ...
                def docKindId = UuidProvider.fromString("a36454de-7af4-11ea-b49f-8b8e95b826ef")
                def name = "Унифицированная накладная (ТОРГ-12)"
                def numeratorSequenceName = "accountdocuniwaybill"
                def numeratorId = UuidProvider.fromString("cc2a4374-7af4-11ea-b4a0-afa6b4c720cb")
