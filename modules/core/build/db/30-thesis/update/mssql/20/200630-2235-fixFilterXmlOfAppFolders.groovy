...
postUpdate.add({
    ...
    try {
        procAppFolders = persistence.getEntityManager().createQuery('''
                ...
                where f.filterXml is not null''', TsProcAppFolder.class)
                .getResultList()
        tx.commit()
    } catch (Exception e) {
    ...
    if (CollectionUtils.isNotEmpty(procAppFolders)) {
        procAppFolders.each { f ->
            appFolderMigrationBean.updateFilterXml(f)
        }
        logger.info("Updated filterXml of ${procAppFolders.size()} app folders")
    }
})