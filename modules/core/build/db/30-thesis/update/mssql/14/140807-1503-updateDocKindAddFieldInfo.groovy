...
postUpdate.add({
    ...
    try {
        List<DocKind> docKinds = em.createQuery('select d from df$DocKind d')
                ...
                .addProperty('docType', new View(CardType.class).addProperty('name').addProperty('fieldsXml')))
                .getResultList()

        Map<CardType, DocKind> docTypeDocKinds = docKinds.groupBy { it.docType } as Map<CardType, DocKind>
        for (CardType docType : docTypeDocKinds.keySet()) {
