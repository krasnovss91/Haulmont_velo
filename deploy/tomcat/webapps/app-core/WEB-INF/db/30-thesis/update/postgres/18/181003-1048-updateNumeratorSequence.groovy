    ...
    try {
        ...
        if (numeratorsToUpdate) {
            numeratorsToUpdate.each {
                ...
                List<String> sequencesToUpdate = em.createNativeQuery("select relname from pg_class " +
                        "where relkind = 'S' and lower(relname) like 'seq_un_$oldSequenceName%'")
                        .getResultList()
                ...
                sequenceRenameMap.each { oldName, newName ->
                    if (selectSequenceName(newName))
                        ...
                        String renameSequenceSql = "alter sequence $oldName rename to $newName"
