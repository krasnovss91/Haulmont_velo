...
alter table TS_CARD_ACL add column TEMPLATE_ID uuid;
...
insert into TS_CARD_ACL (ID, CREATE_TS, CREATED_BY, ENTITY, USER_ID, CARD_ROLE_ID, ORGANIZATION_ID, GLOBAL, DESCRIPTION, DEPARTMENT_ID, CODE, BASE_ACL_ID, TEMPLATE_ID)
  ...
  from TS_CARD_ACL as acl where exists (select * from DF_DOC as d where d.CARD_ID = acl.CARD_ID and d.IS_TEMPLATE = true) and acl.CODE = 4;