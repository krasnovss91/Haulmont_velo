--$Id$
--Description: update department tree function - do not consider deleted DF_EMPLOYEE_DEPARTMENT_POSITION relations
	...
	LOOP
		...
		LOOP
			...
			FOR i IN 1..array_upper(ids,1) LOOP
				FOR userId in
				select e.user_id from df_employee e
				inner join df_employee_department_position_relation edr
				on edr.employee_id=e.correspondent_id where  edr.department_id = ids[i]::uuid and edr.delete_ts is null
				LOOP
