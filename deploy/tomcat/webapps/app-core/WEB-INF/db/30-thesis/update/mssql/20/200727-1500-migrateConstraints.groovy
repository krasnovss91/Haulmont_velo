...
class ConstraintsMigrationHelper {
    ...
    void removeDocsEffectiveConstraints(List<Constraint> constraints, List<String> result) {
        ...
        for (Constraint constraint : toRemove) {
            ...
            if (StringUtils.isBlank(constraint.getJoinClause()) && isSameExpr(constraint.getWhereClause(), expectedWhere)) {
    ...
    String getExpectedEffectiveDocWhereClause(Constraint constraint) {
        ...
        if (ARCHIVIST_GROUP_ID.equals(groupId))
            return '{E}.docOfficeData.officeFile.state >= 30 and {E}.template = false or exists (select acl.id from ts$CardAcl acl where acl.card.id = {E}.id and (acl.user.id = :session$userId or acl.global = true))';

        if (HEAD_OF_DEP_GROUP_ID.equals(groupId) || HEAD_OF_PODR_GROUP_ID.equals(groupId))
            return 'exists (select acl.id from ts$CardAcl acl where acl.card.id={E}.id and (acl.user.id = :session$userId or acl.department.id in (:session$departmentIds) or acl.global = true))';

        return 'exists (select acl.id from ts$CardAcl acl where acl.card.id={E}.id and (acl.user.id = :session$userId or acl.global = true))';
    }
    ...
    void migrateArchivistConstraints(List<Constraint> constraints, List<String> result) {
        List<Constraint> archivistConstraints = selectConstraints(constraints,
                ...
                byBlankJoinClause(),
                byWhereClause('{E}.docOfficeData.officeFile.state >= 30 and {E}.template = false or exists (select acl.id from ts$CardAcl acl where acl.card.id = {E}.id and (acl.user.id = :session$userId or acl.global = true))')
        );
        setNewJoinAndWhereClause(archivistConstraints, result,
                ...
                'file.state >= 30 and {E}.template = false or {E}.id in (select acl.card.id from ts$CardAcl acl where acl.user.id = :session$userId or acl.global = true)');
    }
    ...
    void migrateLimitedAccessAndContractsConstraints(List<Constraint> constraints, List<String> result) {
        List<Constraint> docConstraints = selectConstraints(constraints,
                ...
                byEntityName('df$Doc'),
                byBlankJoinClause(),
                byWhereClause('TYPE({E}) in (:session$contractEffectiveClass) or exists (select acl.id from ts$CardAcl acl where acl.card.id={E}.id and (acl.user.id = :session$userId or acl.global = true))')
        );
        setNewJoinAndWhereClause(docConstraints, result,
                null,
                'TYPE({E}) in (:session$contractEffectiveClass) or exists (select acl.id from ts$CardAcl acl where acl.card.id = {E}.id and (acl.user.id = :session$userId or acl.global = true))');
    }
    ...
    void migrateLimitedAccessAndDocsConstraints(List<Constraint> constraints, List<String> result) {
        List<Constraint> docConstraints = selectConstraints(constraints,
                ...
                byEntityName('df$Doc'),
                byBlankJoinClause(),
                byWhereClause('TYPE({E}) in (:session$simpleDocEffectiveClass, :session$accountDocEffectiveClass) or exists (select acl.id from ts$CardAcl acl where acl.card.id={E}.id and (acl.user.id = :session$userId or acl.global = true))')
        );
        setNewJoinAndWhereClause(docConstraints, result,
                null,
                'TYPE({E}) in (:session$simpleDocEffectiveClass, :session$accountDocEffectiveClass) or exists (select acl.id from ts$CardAcl acl where acl.card.id = {E}.id and (acl.user.id = :session$userId or acl.global = true))');
    }
    ...
    void migrateHeadGroupsConstraints(List<Constraint> constraints, List<String> result) {
        ...
        List<Constraint> taskConstraints = selectConstraints(headConstraints,
                byBlankJoinClause(),
                byWhereClause('exists (select acl.id from ts$CardAcl acl where acl.card.id={E}.id and (acl.department.id in (:session$departmentIds) or acl.user.id = :session$userId))')
        );
        ...
        List<Constraint> docConstraints = selectConstraints(headConstraints,
                byBlankJoinClause(),
                byWhereClause('exists (select acl.id from ts$CardAcl acl where acl.card.id={E}.id and (acl.user.id = :session$userId or acl.department.id in (:session$departmentIds) or acl.global = true))')
        );
        setNewJoinAndWhereClause(docConstraints, result,
                null,
    ...
    void migrateTaskConstraintsAndOtherSame(List<Constraint> constraints, List<String> result) {
        List<Constraint> simpleCardConstraints = selectConstraints(constraints,
                byBlankJoinClause(),
                byWhereClause('exists (select acl.id from ts$CardAcl acl where acl.card.id={E}.id and acl.user.id = :session$userId)')
        );
    ...
    void migrateTaskPatternConstraints(List<Constraint> constraints, List<String> result) {
        List<Constraint> taskPatternConstraints = selectConstraints(constraints,
                byBlankJoinClause(),
                byWhereClause('exists (select acl.id from ts$CardAcl acl where acl.card.id={E}.id and (acl.user.id = :session$userId or acl.global = true))')
        );
    ...
    protected Predicate<Constraint> byBlankJoinClause() {
        return { c -> StringUtils.isBlank(c.getJoinClause()) };
    }
